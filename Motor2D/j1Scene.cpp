...
bool j1Scene::PreUpdate()
{
	...
	if (App->input->GetMouseButtonDown(SDL_BUTTON_LEFT) == KEY_DOWN)
	{
		...
		App->attackManager->AddPropagationAttack(
			...
			ELEMENTAL_TYPE::POISON_ELEMENT, // if the attack has any extra elemental dmg type (if the attack is dmgType=direct, the elemental probability of dmg is calculated by the buff manager)
			100, // base attack damage
			13, // radius (on subtile units)
			40, // propagation speed, in ms (time between steps)
