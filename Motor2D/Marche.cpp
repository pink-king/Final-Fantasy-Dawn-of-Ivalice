...
Marche::Marche(int posX, int posY) : PlayerEntity(posX, posY)
{
	...
	coolDownData.basic.cooldownTime = 0; // warning for encadenate the second time timer max
...
bool Marche::Update(float dt)
{
	...
	case combatState::SPECIAL1:
		if (coolDownData.special1.timer.Read() > coolDownData.special1.cooldownTime)
		{
			...
			App->entityFactory->CreateArrow(App->entityFactory->player->GetSelectedCharacterEntity()->GetThrowingPos(), App->entityFactory->player->GetCrossHairPivotPos().Return_fPoint(), 75, App->entityFactory->player->GetMarche(),PROJECTILE_TYPE::TORNADO);
			// add gui clock
