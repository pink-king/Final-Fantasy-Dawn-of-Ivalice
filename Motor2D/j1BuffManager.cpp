...
{
	for (pugi::xml_node nodeBuff = node.child("buff"); nodeBuff; nodeBuff = nodeBuff.next_sibling("buff"))
	{
		CreateBuff((BUFF_TYPE)nodeBuff.attribute("type").as_int(), (ELEMENTAL_TYPE)nodeBuff.attribute("element").as_int(), (ROL)nodeBuff.attribute("rol").as_int(), App->entityFactory->player, " ", nodeBuff.attribute("value").as_float());
	}
	return true;
}
...
bool j1BuffManager::LoadPortal(pugi::xml_node &node)
{
	Load(node);
	return true;
}

bool j1BuffManager::SavePortal(pugi::xml_node &node) const
{
	Save(node);
	return true;
}
