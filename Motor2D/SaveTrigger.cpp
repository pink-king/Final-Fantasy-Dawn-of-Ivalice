...
{
	isActive = true;
	if (!App->scene->pausePanel->enable && !App->scene->controlsPanel->enable && !App->scene->inventory->enable && !App->scene->settingPanel->enable && App->input->GetControllerGeneralPress(App->input->gamepadScheme.sharedInput.interact) == KEY_DOWN)
	{
		isActive = true;
